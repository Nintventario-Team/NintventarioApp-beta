...
void main() {
  group('DateSelectorWidget', () {
    ...
    testWidgets('displays initial selected date', (WidgetTester tester) async {
      ...
      expect(
          find.text('${initialDate.toLocal()}'.split(' ')[0]), findsOneWidget);
    });
    ...
    testWidgets('calls onDateSelected with the picked date',
        (WidgetTester tester) async {
      DateTime? selectedDate;
      await tester
          .pumpWidget(createWidgetUnderTest(onDateSelected: (DateTime date) {
        selectedDate = date;
      }));
      ...
      expect(selectedDate,
          equals(DateTime(pickedDate.year, pickedDate.month, pickedDate.day)));
    });
  });
}
